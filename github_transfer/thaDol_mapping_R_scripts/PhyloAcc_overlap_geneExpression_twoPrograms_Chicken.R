...
BiocManager::install("org.Gg.eg.db", force=TRUE)
...
##
## FOREGROUND (fg_symbols_filt):
##   gene_name in overlap list, supported by ≥ 2 methods,
##   and present in the expression-based background universe.
##
## BACKGROUND (bg_symbols):
##   OGs expressed in Brain/Heart/Kidney/Liver/Muscle (from dds),
##   mapped via og_rep_all to chicken gene_name symbols.
##
## This version runs GO BP enrichment using CHICKEN ontology:
##   OrgDb = org.Gg.eg.db, keyType = "SYMBOL"
############################################################
...
## Output files (CHICKEN version)
out_prefix   <- "/n/netscratch/edwards_lab/Lab/kelsielopez/Thamnophilus/cactus-snakemake/thamnophilus-all-species-cactus/thamnophilus-all-species-cactus_output/phyloacc_test_900k_output/GO_overlap_phyloacc_expression_2methods_CHICKEN"
out_table    <- paste0(out_prefix, "_BP_Gg_enrichment.csv")
out_dotplot  <- paste0(out_prefix, "_BP_Gg_dotplot.png")
...
## Save raw foreground gene list (for inspection)
...
## -------------------------
## 7. Run enrichGO (BP, CHICKEN)
##    Using SYMBOL as keyType, OrgDb = org.Gg.eg.db
## -------------------------

ego <- tryCatch(
  enrichGO(
    gene          = fg_symbols_filt,
    universe      = bg_symbols,
    OrgDb         = org.Gg.eg.db,
    keyType       = "SYMBOL",
    ont           = "BP",
    ...
    pvalueCutoff  = 0.05,
    qvalueCutoff  = 0.05,
    readable      = TRUE   # converts Entrez IDs to symbols if needed
  ),
  error = function(e) {
...
if (nrow(ego_df) == 0) {
  ...
  cat("Wrote GO BP CHICKEN enrichment table to:\n  ", out_table, "\n\n")
}

## -------------------------
## 8. Dotplot (RStudio + PNG)
## -------------------------

if (nrow(ego_df) > 0) {
  p_dot <- dotplot(ego, showCategory = 20) +
    ggtitle("GO BP – phyloacc ∩ expression (≥2 methods, CHICKEN)") +
    theme_bw() +
    theme(
  ...
  cat("Wrote GO BP dotplot (CHICKEN) to:\n  ", out_dotplot, "\n\n")
}

cat("Done (CHICKEN GO BP enrichment).\n")
