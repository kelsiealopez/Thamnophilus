...
tissue_of_interest <- "Muscle"   # change to "Brain","Heart","Liver","Muscle" etc.
...
mb_labels <- unique(moduleLabels[moduleColors == "midnightblue"])
mb_labels
# Often this will be a single number, e.g. 14

if (length(mb_labels) != 1) {
  stop("Expected exactly one numeric label for greeyellow, found: ",
       paste(mb_labels, collapse = ", "))
}

mb_label <- mb_labels[1]
mb_ME_name <- paste0("ME", mb_label)
mb_ME_name

# Confirm this ME exists
if (!mb_ME_name %in% names(MEs)) {
  stop("Eigengene ", mb_ME_name, " not found in names(MEs). Check labels.")
}

# 2a. Correlation of greeyellow eigengene with Dry
mb_cor <- modTraitCor[mb_ME_name, "Dry"]
mb_p   <- modTraitP[mb_ME_name, "Dry"]
cat("Muscle:", mb_ME_name, "(color = greeyellow) vs Dry: r =",
    round(mb_cor, 3), ", p =", signif(mb_p, 3), "\n")
...
boxplot(ME ~ Dry, data = df_me,
        names = c("Mesic", "Dry"),
        main  = paste("Muscle", mb_ME_name, "(greenyellow) by Dry vs Mesic"),
...
mb_MM_col <- which(names(geneModuleMembership) == paste0("MM_", mb_label))
if (length(mb_MM_col) != 1) {
  stop("Could not uniquely find MM column for label ", mb_label)
}

mb_module_genes <- moduleColors == "midnightblue"
...
verboseScatterplot(
  ...
  ylab = "Gene significance for Dry",
  main = paste("Muscle greeyellow (", gy_ME_name, ") – MM vs GS_Dry", sep = ""),
